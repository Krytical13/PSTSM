...
<#
    ...
    Run:  Invoke-Pester -Path .\PSTaskBuilder.Tests.ps1
...
BeforeAll {
    $modulePath = Join-Path (Split-Path $PSScriptRoot -Parent) 'PSTaskBuilder.psd1'
...
Describe 'Get-PSTaskScriptProfile' {
    Context 'a standard parameterised script' {
        BeforeAll { $script:p = Get-PSTaskScriptProfile -Path $script:StandardScript }
    ...
    Context 'a script that demands elevation and prompts' {
        BeforeAll { $script:a = Get-PSTaskScriptProfile -Path $script:AdminScript }
    ...
    Context 'a script that requires PowerShell 7' {
        BeforeAll { $script:c = Get-PSTaskScriptProfile -Path $script:CoreScript }
    ...
    It 'throws on a path that does not exist' {
        { Get-PSTaskScriptProfile -Path (Join-Path $TestDrive 'nope.ps1') } | Should -Throw '*not found*'
    }
}
...
Describe 'ConvertTo-PSTaskQuotedValue' {
    ...
    It 'leaves numbers bare' {
        ConvertTo-PSTaskQuotedValue -Value 14 | Should -Be '14'
        ConvertTo-PSTaskQuotedValue -Value -3 | Should -Be '-3'
    }
    It 'quotes anything containing whitespace' {
        ConvertTo-PSTaskQuotedValue -Value 'has space' | Should -Be '"has space"'
    }
    It 'doubles a trailing backslash so it cannot escape the closing quote' {
        # Without this, C:\Logs\ swallows the next argument.
        ConvertTo-PSTaskQuotedValue -Value 'C:\Logs\' -AlwaysQuote | Should -Be '"C:\Logs\\"'
    }
    It 'escapes embedded double quotes' {
        ConvertTo-PSTaskQuotedValue -Value 'said "hi"' | Should -Be '"said \"hi\""'
    }
    It 'renders an empty value as an explicit empty string' {
        ConvertTo-PSTaskQuotedValue -Value '' | Should -Be '""'
    }
    It 'quotes on demand even when the value looks safe' {
        ConvertTo-PSTaskQuotedValue -Value 'safe' -AlwaysQuote | Should -Be '"safe"'
    }
    It 'keeps $true unquoted so it does not arrive as the string "True"' {
        ConvertTo-PSTaskQuotedValue -Value $true | Should -Be '$true'
    }
}

Describe 'ConvertTo-PSTaskArgument' {
    BeforeAll {
        $script:args1 = ConvertTo-PSTaskArgument -ScriptPath 'C:\My Scripts\Run.ps1' -Parameters ([ordered]@{
    ...
    It 'emits -Switch:$false when the script defaults that switch to true' {
        ...
        $prof = Get-PSTaskScriptProfile -Path $script:AdminScript
        $a = ConvertTo-PSTaskArgument -ScriptPath $script:AdminScript `
            -Parameters ([ordered]@{ Verbose2 = $false }) -ScriptProfile $prof
        $a | Should -BeLike '*-Verbose2:$false*'
    }

    It 'honours switch suppression' {
        $a = ConvertTo-PSTaskArgument -ScriptPath 'C:\x.ps1' -NoProfile $false -NonInteractive $false -ExecutionPolicy 'None' -WindowStyle 'None'
        $a | Should -Be '-File "C:\x.ps1"'
    }
}

Describe 'ConvertFrom-PSTaskAction' {
    It 'round-trips what ConvertTo-PSTaskArgument produced' {
        $params = [ordered]@{ SmtpServer = 'mail.contoso.com'; DaysOut = 14; TestMode = $true }
        $rendered = ConvertTo-PSTaskArgument -ScriptPath 'C:\My Scripts\Run.ps1' -Parameters $params

        $parsed = ConvertFrom-PSTaskAction -Execute 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -Arguments $rendered
    ...
    It 'understands abbreviated engine switches that real tasks use' {
        $parsed = ConvertFrom-PSTaskAction -Execute 'pwsh.exe' -Arguments '-nop -noni -ex Bypass -File "C:\a.ps1" -X 1'
    ...
    It 'recovers a script path from the old -Command "& path" form and flags it' {
        $parsed = ConvertFrom-PSTaskAction -Execute 'powershell.exe' -Arguments '-ExecutionPolicy Bypass -Command "& ''C:\Scripts\Old.ps1'' -Mode Fast"'
    ...
    It 'refuses to normalise inline -Command code' {
        $parsed = ConvertFrom-PSTaskAction -Execute 'powershell.exe' -Arguments '-Command "Get-Service | Restart-Service"'
    ...
    It 'reports a non-PowerShell action rather than guessing' {
        $parsed = ConvertFrom-PSTaskAction -Execute 'C:\Windows\System32\cmd.exe' -Arguments '/c echo hi'
        $parsed.IsPowerShell | Should -BeFalse
        $parsed.IsRecognized | Should -BeFalse
    }

    It 'detects the 32-bit host' {
        $parsed = ConvertFrom-PSTaskAction -Execute 'C:\Windows\SysWOW64\WindowsPowerShell\v1.0\powershell.exe' -Arguments '-File "C:\a.ps1"'
        $parsed.Bitness | Should -Be 'x86'
    }

    It 'treats a trailing bare switch as $true' {
        $parsed = ConvertFrom-PSTaskAction -Execute 'powershell.exe' -Arguments '-File "C:\a.ps1" -Force'
        $parsed.Parameters['Force'] | Should -BeTrue
    }

    It 'reads the -Switch:$false form' {
        $parsed = ConvertFrom-PSTaskAction -Execute 'powershell.exe' -Arguments '-File "C:\a.ps1" -Force:$false'
        $parsed.Parameters['Force'] | Should -BeFalse
    }
}

Describe 'ConvertFrom-PSTaskDuration' {
    It 'converts ISO 8601 durations Task Scheduler stores' {
        ConvertFrom-PSTaskDuration -Value 'PT4H' | Should -Be '04:00:00'
        ConvertFrom-PSTaskDuration -Value 'PT15M' | Should -Be '00:15:00'
        ConvertFrom-PSTaskDuration -Value 'PT30S' | Should -Be '00:00:30'
        ConvertFrom-PSTaskDuration -Value 'P1D' | Should -Be '1.00:00:00'
        ConvertFrom-PSTaskDuration -Value 'PT1H30M' | Should -Be '01:30:00'
    }
    It 'returns null rather than throwing on absent or malformed input' {
        ConvertFrom-PSTaskDuration -Value $null | Should -BeNullOrEmpty
        ConvertFrom-PSTaskDuration -Value '' | Should -BeNullOrEmpty
        ConvertFrom-PSTaskDuration -Value 'garbage' | Should -BeNullOrEmpty
    }
}

Describe 'New-PSTaskTriggerSpec' {
    It 'normalises a bare time to a full start boundary' {
        (New-PSTaskTriggerSpec -Type Daily -At '07:00').At | Should -BeLike '*T07:00:00'
    }
    It 'requires a time for time-based triggers' {
        { New-PSTaskTriggerSpec -Type Daily } | Should -Throw '*requires -At*'
    }
    ...
    It 'rejects a repetition duration with no interval' {
        { New-PSTaskTriggerSpec -Type Daily -At '07:00' -RepetitionDuration '01:00:00' } | Should -Throw '*requires RepetitionInterval*'
    }
    It 'rejects an unparseable timespan' {
        { New-PSTaskTriggerSpec -Type Daily -At '07:00' -RandomDelay 'soon' } | Should -Throw '*not a valid timespan*'
    }
    It 'accepts a startup trigger with no time' {
        (New-PSTaskTriggerSpec -Type AtStartup -Delay '00:02:00').Type | Should -Be 'AtStartup'
    }
}

Describe 'New-PSTaskPlan' {
    It 'fills the plan from the script with no other input' {
        $plan = New-PSTaskPlan -ScriptPath $script:StandardScript
    ...
    It 'lets explicit parameters win' {
        $plan = New-PSTaskPlan -ScriptPath $script:StandardScript -Parameters ([ordered]@{ DaysOut = 30 })
        $plan.Parameters['DaysOut'] | Should -Be 30
    }
    It 'raises RunLevel automatically when the script requires elevation' {
        (New-PSTaskPlan -ScriptPath $script:AdminScript).Principal.RunLevel | Should -Be 'Highest'
        (New-PSTaskPlan -ScriptPath $script:StandardScript).Principal.RunLevel | Should -Be 'Limited'
    }
    It 'applies reliability defaults that differ from Task Scheduler own' {
        $s = (New-PSTaskPlan -ScriptPath $script:StandardScript).Settings
    ...
    It 'normalises the task folder' {
        (New-PSTaskPlan -ScriptPath $script:StandardScript -TaskPath 'Custom').TaskPath | Should -Be '\Custom\'
    }
...
Describe 'Test-PSTaskPlan' {
    BeforeAll {
        $script:basePlan = New-PSTaskPlan -ScriptPath $script:StandardScript `
            -Parameters ([ordered]@{ SmtpServer = 'mail.contoso.com' }) `
...
Describe 'Export-PSTaskPlan / Import-PSTaskPlan' {
    It 'round-trips a plan through JSON without losing anything that matters' {
        $plan = New-PSTaskPlan -ScriptPath $script:StandardScript `
            ...
            -TaskPath 'Custom' `
            -Trigger (New-PSTaskTriggerSpec -Type Weekly -At '18:30' -DaysOfWeek Monday, Thursday)

        $file = Join-Path $TestDrive 'plan.json'
        Export-PSTaskPlan -Plan $plan -Path $file

        $back = Import-PSTaskPlan -Path $file
...
"@ | Set-Content -LiteralPath $payload -Encoding UTF8
        ...
        $plan = New-PSTaskPlan -ScriptPath $payload -Parameters ([ordered]@{
                Label   = 'hello world'
                Count   = 7
                DryRun  = $true
                OutPath = 'C:\Logs\'
        ...
        $wrapper = New-PSTaskLogWrapper -Plan $plan
        $argString = ConvertTo-PSTaskArgument -ScriptPath $wrapper -Parameters $plan.Parameters

        $engine = (Get-PSTaskEngine -Id 'powershell' | Where-Object Bitness -eq 'x64' | Select-Object -First 1).Path
...
Describe 'Get-PSTaskEngine' {
    ...
    It 'marks exactly one engine as the default' {
        @(Get-PSTaskEngine | Where-Object IsDefault) | Should -HaveCount 1
    }
    It 'filters by id' {
        @(Get-PSTaskEngine -Id 'powershell' | Where-Object { $_.Id -ne 'powershell' }) | Should -HaveCount 0
    }
}

Describe 'ConvertFrom-PSTaskResultCode' {
    It 'decodes the codes people misread' {
        ConvertFrom-PSTaskResultCode -Code 0x41303 | Should -BeLike '*never run*'
        ConvertFrom-PSTaskResultCode -Code 0x41301 | Should -BeLike '*currently running*'
        ConvertFrom-PSTaskResultCode -Code 0 | Should -BeLike '*Success*'
        ConvertFrom-PSTaskResultCode -Code 2 | Should -BeLike '*File not found*'
    }
    It 'falls back to hex for anything unmapped' {
        ConvertFrom-PSTaskResultCode -Code 0x12345 | Should -Be '0x12345'
    }
    It 'returns null for no result' {
        ConvertFrom-PSTaskResultCode -Code $null | Should -BeNullOrEmpty
    }
}
